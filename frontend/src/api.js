...
export const api = {
  ...
  updateWorkStatus: (id, status) => request(`/works/${id}/status`, { method: 'PUT', body: JSON.stringify({ status }) }),
  getAgencies: () => request('/agencies'),
  getConflicts: () => request('/conflicts'),
  getOpportunities: () => request('/coordination/opportunities'),
  analyzeCoordination: (data) => request('/coordination/analyze', { method: 'POST', body: JSON.stringify(data) }),
  getProposals: () => request('/coordination/proposals'),
  createProposal: (data) => request('/coordination/proposals', { method: 'POST', body: JSON.stringify(data) }),
  updateProposal: (id, data) => request(`/coordination/proposals/${id}`, { method: 'PUT', body: JSON.stringify(data) }),
  getRoadHistory: (roadName) => request(`/roads/${encodeURIComponent(roadName)}/history`),
