...
function checkHardConstraints(works) {
  ...
  for (const work of works) {
    if (work.priority === 'Emergency') {
      violations.push({ work_id: work.id, constraint: 'Emergency project', type: 'hard' });
    }
    const start = parseDate(work.start_date);
    const end = parseDate(work.end_date);
    if (end < start) {
      violations.push({ work_id: work.id, constraint: 'Invalid dates', type: 'hard' });
    }
  }
  ...
  if (commonEnd < commonStart) {
    violations.push({ constraint: 'Completely incompatible schedule', type: 'hard' });
  }
...
function getRoadHistoryScore(roadName, history) {
  const roadHistory = history.filter(h => h.road_name.toLowerCase() === roadName.toLowerCase());
  if (roadHistory.length === 0) return { score: 5, reason: 'No recent road history' };
  if (roadHistory.length >= 3) return { score: 15, reason: 'Road has upcoming excavation activity' };
  if (roadHistory.length >= 1) return { score: 12, reason: 'Road has recent intervention history' };
...
function getScheduleScore(works) {
  ...
  return { score, reason: `${Math.round(avgOverlap)}-day schedule overlap`, overlapDays: avgOverlap };
}
...
function getDurationScore(works) {
  ...
  if (ratio >= 0.4) return { score: 7, reason: 'Moderately compatible duration' };
...
function getTrafficScore(works) {
  ...
  const partialClosures = works.filter(w => w.closure_type === 'Partial').length;
  if (fullClosures >= 2) return { score: 4, reason: 'Multiple full road closures' };
  if (fullClosures === 1 && partialClosures >= 1) return { score: 6, reason: 'Mixed closure types' };
  if (partialClosures >= 2) return { score: 7, reason: 'Partial closures manageable' };
...
function getPriorityScore(works) {
  ...
  if (priorities.includes('Emergency')) return { score: 2, reason: 'Emergency priority present' };
  const highCount = priorities.filter(p => p === 'High').length;
  if (highCount >= 2) return { score: 6, reason: 'Multiple high-priority projects' };
...
function analyzeCoordination(works, roadHistory = []) {
  ...
  const hasEmergencyOnly = hardViolations.every(v => v.constraint === 'Emergency project');
  const hasBlockingHard = hardViolations.some(v =>
    v.constraint === 'Invalid dates' || v.constraint === 'Completely incompatible schedule'
  );
  ...
  let total = spatial.score + schedule.score + history.score + duration.score + traffic.score + priority.score;
  ...
  if (traffic.score >= 7) reasons.push('✓ Reduced traffic disruption potential');
  ...
  return {
    ...
    breakdown: {
      spatial: { score: spatial.score, max: 30, label: 'Spatial compatibility' },
      schedule: { score: schedule.score, max: 25, label: 'Schedule compatibility' },
      history: { score: history.score, max: 15, label: 'Road history' },
      duration: { score: duration.score, max: 10, label: 'Duration compatibility' },
      traffic: { score: traffic.score, max: 10, label: 'Traffic impact' },
      priority: { score: priority.score, max: 10, label: 'Priority compatibility' }
    },
    reasons,
...
function generateScenarios(works, roadHistory = []) {
  ...
  scenarios.push({
    ...
    works: works.map(w => ({ id: w.id, work_type: w.work_type, start: w.start_date, end: w.end_date })),
...
function findCoordinationOpportunities(works, roadHistory = []) {
  ...
  for (const conflict of conflicts) {
    ...
    opportunities.push({
      ...
      analysis,
      scenarios,
      recommended_scenario: scenarios.find(s => s.recommended) || scenarios[0]
    });
  }

  return opportunities.sort((a, b) => b.coordination_score - a.coordination_score);
}
